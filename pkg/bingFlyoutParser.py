...
class Reward:
    ...
    class Type:
        ...
        SEARCH_AND_EARN_DESCR_RE = re.compile("Earn (?P<count>\d+) credits? per (?P<cost>\d+) Bing search(es)? up to (?P<max>\d+) credits? a day")
        EARN_CREDITS_RE = re.compile("Earn (?P<credits>\d+) credits?")
